...
func InitFlags() {
	...
	flag.Usage = func() {
		fmt.Println("Usage: pleach [OPTION]... [FILE|DIR]... [+LINE[:COL]] [+/REGEX]")
		fmt.Println("       pleach [OPTION]... [FILE[:LINE[:COL]]]...  (only if the `parsecursor` option is enabled)")
		fmt.Println("-clean")
...
func DoEvent() {
	...
	if event != nil {
		...
		if resize {
			...
			action.Tabs.HandleEvent(event)
		} else if action.HandleOverlayEvent(event, action.InfoBar.HasPrompt) {
			// command bar or popup menu consumed the event
