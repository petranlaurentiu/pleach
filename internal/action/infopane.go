...
// FindNextMatch jumps to the next match without closing find.
func (h *InfoPane) FindNextMatch() {
	if isFindPrompt(h.PromptType) {
		commitFind(true)
		return
	}
...
// FindPrevMatch jumps to the previous match without closing find.
func (h *InfoPane) FindPrevMatch() {
	if isFindPrompt(h.PromptType) {
		commitFind(false)
	}
}
...
var InfoKeyActions = map[string]InfoKeyAction{
	...
	"FindNextMatch":     (*InfoPane).FindNextMatch,
	"FindPrevMatch":     (*InfoPane).FindPrevMatch,
