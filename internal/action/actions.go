...
func (h *BufPane) MousePress(e *tcell.EventMouse) bool {
	b := h.Buf
	mx, my := e.Position()
	if w, ok := h.BWindow.(*display.BufWindow); ok && w.HitCloseButton(mx, my) {
		closeWindow(h)
		return true
	}
...
// Find opens a VS Code-style find prompt (literal text).
func (h *BufPane) Find() bool {
	return startFind(h, false)
}

// FindLiteral is the same as Find.
func (h *BufPane) FindLiteral() bool {
	return startFind(h, false)
}

// FindRegex opens find using regular expressions.
func (h *BufPane) FindRegex() bool {
	return startFind(h, true)
}
...
func (h *BufPane) FindNext() bool {
	...
	if currentFind.active {
		updateFindPrompt(h, h.Buf.LastSearch, h.Buf.LastSearchRegex)
	}
	return true
}
