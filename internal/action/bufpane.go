...
func (h *BufPane) HandleEvent(event tcell.Event) {
	...
	switch e := event.(type) {
	case *tcell.EventRaw:
		if ke, ok := decodeExtendedKey(e.EscSeq()); ok {
			h.DoKeyEvent(ke)
			break
		}
		re := RawEvent{
...
var BufKeyActions = map[string]BufKeyAction{
	...
	"InsertLineAfter":           (*BufPane).InsertLineAfter,
	"InsertLineBefore":          (*BufPane).InsertLineBefore,
	...
	"FormatDocument":            (*BufPane).FormatDocument,
	...
	"OpenTerminal":              (*BufPane).OpenTerminalAction,
	"OpenAgent":                 (*BufPane).OpenAgentAction,
	"ClosePanel":                (*BufPane).ClosePanelAction,
	"ShowLayoutMenu":            (*BufPane).ShowLayoutMenu,
	"ShowOptionsMenu":           (*BufPane).ShowOptionsMenu,
	"MovePaneRight":             (*BufPane).MovePaneRight,
	"MovePaneLeft":              (*BufPane).MovePaneLeft,
	"MovePaneBelow":             (*BufPane).MovePaneBelow,
	"MovePaneAbove":             (*BufPane).MovePaneAbove,
...
var BufMouseActions = map[string]BufMouseAction{
	...
	"ContextMenu":      (*BufPane).ContextMenu,
}
