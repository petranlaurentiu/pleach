...
func Init() error {
	...
	// Turn off leftover kitty/xterm key protocols from older Pleach builds.
	// Those encodings broke Ctrl+Z in Ghostty.
	disableExtendedKeys()
	return nil
}

// Fini shuts down the screen.
func Fini() {
	disableExtendedKeys()
	if Screen != nil {
		Screen.Fini()
