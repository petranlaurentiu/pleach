...
// DrawCommandBar, when set, replaces the default key cheat-sheet with clickable buttons.
var DrawCommandBar func(width, infoY int, style tcell.Style)

func (i *InfoWindow) displayKeyMenu() {
	if DrawCommandBar != nil {
		DrawCommandBar(i.Width, i.Y, i.defStyle())
		return
	}
...
func (i *InfoWindow) Display() {
	...
	if i.HasSuggestions && len(i.Suggestions) > 1 {
		...
		keymenuOffset := 0
		if config.GetGlobalOption("keymenu").(bool) {
			keymenuOffset = config.CommandBarRows
		}
