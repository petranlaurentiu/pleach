...
func (s *StatusLine) Display() {
	...
	winX := s.win.X
	s.win.closeBtnX, s.win.closeBtnY, s.win.closeBtnW = 0, 0, 0
	...
	closeBtn := []byte(" ×")
	closeW := util.StringWidth(closeBtn, util.CharacterCount(closeBtn), 1)
	innerW := s.win.Width - closeW
	if innerW < 0 {
		innerW = 0
		closeW = s.win.Width
	}
	...
	for x := 0; x < innerW; x++ {
		...
		} else if x >= innerW-rightLen && x < innerW {
	...
	s.win.closeBtnX = winX + innerW
	s.win.closeBtnY = y
	s.win.closeBtnW = closeW
	for i := 0; i < closeW && len(closeBtn) > 0; i++ {
		r, combc, size := util.DecodeCharacter(closeBtn)
		closeBtn = closeBtn[size:]
		screen.SetContent(winX+innerW+i, y, r, combc, statusLineStyle)
	}
}
