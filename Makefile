.PHONY: runtime build generate build-quick herdr
...
build-quick:
	CGO_ENABLED=$(CGO_ENABLED) go build -o pleach -trimpath -ldflags "-s -w $(GOVARS) $(ADDITIONAL_GO_LINKER_FLAGS)" ./cmd/micro

# Download the official Herdr binary next to ./pleach (used by the Agent panel).
herdr:
	@set -e; \
	...
	case "$$os-$$arch" in \
	  ...
	  *) echo "No herdr binary for $$os $$arch"; exit 1 ;; \
	esac; \
	curl -fsSL -o herdr "https://github.com/herdrdev/herdr/releases/latest/download/$$asset"; \
	chmod +x herdr; \
	echo "Installed ./herdr"

build-dbg:
	CGO_ENABLED=$(CGO_ENABLED) go build -o pleach -trimpath -ldflags "$(ADDITIONAL_GO_LINKER_FLAGS) $(DEBUGVAR)" ./cmd/micro
...
install: generate
	CGO_ENABLED=$(CGO_ENABLED) go build -o $(GOBIN)/pleach -ldflags "-s -w $(GOVARS) $(ADDITIONAL_GO_LINKER_FLAGS)" ./cmd/micro
