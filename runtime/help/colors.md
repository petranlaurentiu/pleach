...
These match Herdr's built-in palettes. Pick one from the toolbar **Theme**
button, or `> set colorscheme catppuccin`.

* `catppuccin` (Pleach default) and `catppuccin-latte`
* `tokyo-night` and `tokyo-night-day`
* `dracula`, `nord`, `vesper`
* `gruvbox-tc` and `gruvbox-light`
* `one-dark` and `one-light`
* `solarized-tc` and `solarized-light`
* `kanagawa` and `kanagawa-lotus`
