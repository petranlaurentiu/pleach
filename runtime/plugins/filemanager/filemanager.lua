...
local function refresh_view()
 ...
 -- VS Code-style header: section label + workspace folder name
 tree_view.Buf.EventHandler:Insert(buffer.Loc(0, 0), "EXPLORER\n")
 tree_view.Buf.EventHandler:Insert(buffer.Loc(0, 1), get_basename(current_dir) .. "\n")
...
-- Click a folder to expand/collapse in place (VS Code). Click a file to
-- open it in the existing editor. ".." goes up one workspace folder.
...
local function try_open_at_y(y)
 ...
 y = y - 2
 if scanlist[y].dirmsg == "+" then
 uncompress_at_cursor()
 elseif scanlist[y].dirmsg == "-" then
 compress_at_cursor()
 elseif scanlist[y].dirmsg == "" then
 micro.OpenInEditor(scanlist[y].abspath)
 else
...
local function open_tree()
 ...
 tree_view.Buf:SetOptionNative("statusformatl", "EXPLORER")
...
-- On click, let Micro move the cursor, then open that line.
-- GetMouseClickLocation was removed from current Micro.
...
local function prompt_new_file()
 micro.InfoBar():Prompt("New file: ", "", "File", nil, function(resp, canceled)
  if not canceled and resp ~= "" then
   create_filedir(resp, false)
  end
 end)
end

local function prompt_new_dir()
 micro.InfoBar():Prompt("New folder: ", "", "File", nil, function(resp, canceled)
  if not canceled and resp ~= "" then
   create_filedir(resp, true)
  end
 end)
end

local function prompt_rename()
 micro.InfoBar():Prompt("Rename: ", "", "File", nil, function(resp, canceled)
  if not canceled and resp ~= "" then
   rename_at_cursor(micro.CurPane(), {resp})
  end
 end)
end

local function copy_path_at_cursor()
 local y = get_safe_y()
 local path = current_dir
 if y ~= 0 and not scanlist_is_empty() then
  path = scanlist[y].abspath
 end
 micro.CopyToClipboard(path)
 micro.InfoBar():Message("Copied ", path)
end

function onContextMenu(view, event)
 if view ~= tree_view then
  return true
 end
 aftermove_if_tree(view)
 local y = get_safe_y()
 local is_folder = y ~= 0 and not scanlist_is_empty() and scanlist[y].dirmsg ~= ""
 local labels
 if y == 0 or scanlist_is_empty() then
  labels = {"New file", "New folder", "Close tree"}
 elseif is_folder then
  labels = {"Expand / Collapse", "Open folder", "New file", "New folder", "Rename", "Delete", "Copy path", "Close tree"}
 else
  labels = {"Open", "New file", "New folder", "Rename", "Delete", "Copy path", "Close tree"}
 end
 micro.ShowMenu("Explorer", labels, function(i)
  if y == 0 or scanlist_is_empty() then
   if i == 1 then
    prompt_new_file()
   elseif i == 2 then
...
function init()
 ...
 config.RegisterCommonOption("filemanager", "foldersfirst", true)
    -- Pleach opens the file tree by default (VS Code-style explorer)
